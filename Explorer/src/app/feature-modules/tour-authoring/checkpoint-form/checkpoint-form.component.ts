...
export class CheckpointFormComponent implements OnChanges, OnInit {
  ...
  private fillAddForm() {
    const hours = this.checkpointForm.value.hours ?? 0;
    const minutes = this.checkpointForm.value.minutes ?? 0;
    const requiredTimeInSeconds = hours * 3600 + minutes * 60;
    ...
    const checkpoint: Checkpoint = {
      ...
      requiredTimeInSeconds: requiredTimeInSeconds, // Use the calculated value
      currentPicture: 0,
      visibleSecret: false,
      showedPicture: "",
      viewSecretMessage: "",
